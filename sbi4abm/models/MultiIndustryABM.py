...
class Model:
	def __init__(self):
		...
		self.config = toml.load(os.path.join(self.base_path,
											 "model_config.toml"))
	...
	def simulate(self, pars=None, T=100, seed=None):
		...
		## it to file
		self.config["expectation_react_param"] = params[1]
		tmpfilename = os.path.join(self.basepath, "sbi4abm_configs",
								   "model_config_" + simulation_number +
								   ".toml")
		with open(tmpfilename, "w") as outputfile:
				toml.dump(self.config, outputfile)

		## TODO start simulation, get output directory
		exit_code = subprocess.call(["julia", ## "juliaclient",
									 os.path.join(self.base_path, "run_one_simulation.jl"),
		...
		if exit_code == 0:
				res_dt = pd.read_parquet(os.path.join(self.base_path,
													  "results",
													  self.experiment_dir,
													  "sim_" + str(simulation_number),
													  "data",
													  "Model.parquet"),
										 columns = ["real_GDP", "unemployment_rate"])
